...
# 2. Define Content for Broken Files
content_map = {
    'destination-ngorongoro.html': {
        ...
        'about_html': '''
                        <p class="mb-4">The Ngorongoro Conservation Area constitutes a UNESCO World Heritage Site located 180 km (110 mi) west of Arusha in the Crater Highlands area of Tanzania. The conservation area is named after Ngorongoro Crater, a large volcanic caldera within the area.</p>
        ...
        'nearby_replace': ('Ngorongoro Crater', '''
            <!-- Serengeti Block -->
    ...
    'destination-manyara.html': {
        ...
        'about_html': '''
                        <p class="mb-4">Lake Manyara National Park is a protected area in Tanzania\'s Arusha and Manyara Regions, covering an area of 325 square kilometers. The park is famous for its unique tree-climbing lions and large flocks of flamingos that paint the lake pink.</p>
    ...
    'destination-terangile.html': {
        ...
        'about_html': '''
                        <p class="mb-4">Tarangire National Park is the sixth largest national park in Tanzania, it is located in Manyara Region. The name of the park originates from the Tarangire River that crosses the park.</p>
        ...
        'nearby_replace': ('Tarangire National Park', '''
            <!-- Serengeti Block -->
            <div class="row align-items-center mb-5">
                <div class="col-lg-6 position-relative">
                    <img src="img/nyumbu-migrate.jpeg" class="img-fluid rounded main-img" alt="Serengeti Plains">
...
def restore_file(filename, data):
    ...
    # 4. Highlights Section (Replace text for 5 items)
    # This is tricky with regex, so we'll do it sequentially
    highlights = data['highlights']
    # Reset helper regex specific to template structure
    # Template has item 1: Great Migration (fa-paw)
    new_content = re.sub(r'<h5>The Great Migration</h5>\s*<p class="mb-0">.*?</p>', f'<h5>{highlights[0][1]}</h5><p class="mb-0">{highlights[0][2]}</p>', new_content, flags=re.DOTALL)
    new_content = re.sub(r'<i class="fa fa-paw"></i>', f'<i class="fa {highlights[0][0]}"></i>', new_content)
    
    # Item 2: Big 5 Safari (fa-camera)
    new_content = re.sub(r'<h5>Big 5 Safari</h5>\s*<p class="mb-0">.*?</p>', f'<h5>{highlights[1][1]}</h5><p class="mb-0">{highlights[1][2]}</p>', new_content, flags=re.DOTALL)
    new_content = re.sub(r'<i class="fa fa-camera"></i>', f'<i class="fa {highlights[1][0]}"></i>', new_content)
    
    # Item 3: Endless Plains (fa-globe)
    new_content = re.sub(r'<h5>Endless Plains</h5>\s*<p class="mb-0">.*?</p>', f'<h5>{highlights[2][1]}</h5><p class="mb-0">{highlights[2][2]}</p>', new_content, flags=re.DOTALL)
    new_content = re.sub(r'<i class="fa fa-globe"></i>', f'<i class="fa {highlights[2][0]}"></i>', new_content)
    
    # Item 4: Balloon Safaris (fa-cloud)
    new_content = re.sub(r'<h5>Balloon Safaris</h5>\s*<p class="mb-0">.*?</p>', f'<h5>{highlights[3][1]}</h5><p class="mb-0">{highlights[3][2]}</p>', new_content, flags=re.DOTALL)
    new_content = re.sub(r'<i class="fa fa-cloud"></i>', f'<i class="fa {highlights[3][0]}"></i>', new_content)
    
    # Item 5: Bird Watching (fa-binoculars)
    new_content = re.sub(r'<h5>Bird Watching</h5>\s*<p class="mb-0">.*?</p>', f'<h5>{highlights[4][1]}</h5><p class="mb-0">{highlights[4][2]}</p>', new_content, flags=re.DOTALL)
    new_content = re.sub(r'<i class="fa fa-binoculars"></i>', f'<i class="fa {highlights[4][0]}"></i>', new_content)
    
    # 5. Top Tours
    new_content = re.sub(r'Experience Serengeti\'s Top Tours', data['tours_title'], new_content)
    # Images (defined in tour_images list)
    # They are in order in the file: nyumbu-migrate.jpeg, pro3.jpg, simba.jpg, photo1.jpeg
    t_imgs = data['tour_images']
    new_content = new_content.replace('src="img/nyumbu-migrate.jpeg"', f'src="img/{t_imgs[0]}"', 1) # Only replace the tour one (careful with hero/nearby)
    # Wait, Hero image was already replaced. Nearby image IS nyumbu-migrate. 
    # The Template Hero is nyumbu-migrate. Loop 1 replaces Hero. 
    # The Template Nearby (Serengeti Block) is NOT in the template - the template lists Ngorongoro/Manyara/Tarangire/Zanzibar.
    # Ah, template doesn't have Serengeti block yet.
    # So replacing 'nyumbu-migrate.jpeg' might affect the first Tour Card.
    # Let's use specific regex for tour cards.
    ...
    # 6. Nearby Destinations (Replace Self-Reference with Serengeti Block)
    start_str = f'<h3 class="dest-country">{data["nearby_replace"][0]}</h3>' 
    # We need to find the whole block containing this h3.
    # Template blocks: Ngorongoro, Lake Manyara, Tarangire, Zanzibar
    
    target_name = data["nearby_replace"][0]
    replacement_block = data["nearby_replace"][1]
    
    # Regex to find the row containing the target name
    pattern = re.compile(r'(<div class="row [^"]+">\s*<div class="col-lg-6 position-relative">.*?<h3 class="dest-country">' + re.escape(target_name) + r'</h3>.*?</div>\s*</div>)', re.DOTALL)
    
    if pattern.search(new_content):
        new_content = pattern.sub(replacement_block, new_content)
    else:
        print(f"Warning: Could not find nearby block for {target_name}")
    ...
    new_content = re.sub(r'<div class="accordion" id="accordionExample">.*?</div>\s*</div>\s*</div>', f'{new_faq_html}\n                </div>\n            </div>', new_content, flags=re.DOTALL)
...
# PROCESS
print("Restoring broken destination files...")
